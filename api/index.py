...
# pyparsing is not thread-safe so we lock SPARQL queries
...
def get_jobs():
    ...
    for row in results:
        ...
        WHERE {
            {
                ...
                FILTER NOT EXISTS { <%s> ex:requiresCoreSkill ?skill }
            } UNION {
        ...
        """ % (str(job_uri), str(job_uri), str(job_uri))
...
def get_predefined_profiles():
    ...
    PREFIX ex: <http://example.org/jobmatch#>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    SELECT ?applicant ?name ?type ?location ?degreeName ?degreeLevel ?fieldOfStudy ?institution ?gradYear ?expRole ?expOrg ?expDuration ?expType
    WHERE {
    ...
    for row in results:
        ...
        p_name = str(row.name)
        p_loc = str(row.location) if hasattr(row, 'location') and row.location else "Kuala Lumpur"
...
def calculate_matches(user_profile):
    ...
    # 1. Load transitive skill implications dynamically from the ontology
    inferred_skills_dict = {}
    query_inferred = """
    PREFIX ex: <http://example.org/jobmatch#>
    SELECT ?parent ?child
    WHERE {
        ?parent ex:impliesSkill ?child .
    }
    """
    for row in safe_query(query_inferred):
        p_name = str(row.parent).split("#")[-1]
        c_name = str(row.child).split("#")[-1]
        if p_name not in inferred_skills_dict:
            inferred_skills_dict[p_name] = set()
        inferred_skills_dict[p_name].add(c_name)
        
    user_skills_set = set(user_profile.get("skills", []))
    inferred_skills = set()
    for skill in user_skills_set:
        if skill in inferred_skills_dict:
            inferred_skills.update(inferred_skills_dict[skill])
    ...
    # 2. Load skill course recommendations dynamically from the ontology
    skill_courses_dict = {}
    query_courses = """
    PREFIX ex: <http://example.org/jobmatch#>
    SELECT ?skill ?course
    WHERE {
        ?skill ex:hasRecommendedCourse ?course .
    }
    """
    for row in safe_query(query_courses):
        s_name = str(row.skill).split("#")[-1]
        course_str = str(row.course)
        skill_courses_dict[s_name] = course_str
        
    # 3. Load Klang Valley cities dynamically from the ontology
    query_kv = """
    PREFIX ex: <http://example.org/jobmatch#>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    SELECT ?label
    WHERE {
        ?loc a ex:KlangValleyLocation ;
             rdfs:label ?label .
    }
    """
    klang_valley_cities = {str(row.label) for row in safe_query(query_kv)}
    if not klang_valley_cities:
    ...
    for job in jobs:
        ...
        # Core skills are loaded directly from the ontology requiresCoreSkill relation
        core_set = {s["id"] for s in job["skills"] if s.get("isCore")}
        if not core_set:
        ...
        # score: core skills = 70%, secondary = 30%
        ...
        # need at least one core skill to qualify as a match
        ...
        for skill_id in missing_core:
            if skill_id in skills_map:
                recommendations.append({
                    ...
                    "course": skill_courses_dict.get(skill_id, "Online Tutorials & Certification Courses"),
                    "type": "Core"
                })
...
LIMIT 5"""
    ...
    if request.method == "POST":
        try:
            # Security check: prevent destructive operations (INSERT, DELETE, CLEAR, etc.)
            query_upper = query_str.upper()
            destructive_keywords = ["DELETE", "INSERT", "LOAD", "CLEAR", "DROP", "CREATE", "ADD", "MOVE", "COPY"]
            if any(k in query_upper for k in destructive_keywords):
                error = "Security Error: Only read-only queries (SELECT, ASK, CONSTRUCT, DESCRIBE) are allowed."
            else:
